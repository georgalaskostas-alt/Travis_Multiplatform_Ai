...
echo "== TRAVIS Runtime V5 acceptance =="
...
h=json.load(open(sys.argv[1]));age=time.time()-float(h.get('lastBeatAt',0));print(json.dumps({k:h.get(k) for k in ('version','generation','pid','state','marketEngine','headlessAI','testnetAdapter')},indent=2));assert int(h.get('version',0))>=11;assert age<8
PY
JID=$("$PYTHON" - <<'PY'
...
with lock.open('a+') as f:
 ...
 except:d={"version":11,"jobs":[]}
 d['jobs'].append({"id":jid,"title":"Runtime V5 Acceptance Mission","kind":"headlessMission","state":"scheduled","createdAt":now,"updatedAt":now,"nextRunAt":now,"payload":{"goal":"Verify lease-safe Always-On runtime","executionMode":"full","plan":[{"order":1,"title":"Runtime identity","capability":"runtime.identity"},{"order":2,"title":"System health","capability":"runtime.health"},{"order":3,"title":"Safety envelope","capability":"runtime.safety"},{"order":4,"title":"Final report","capability":"report.synthesize"}]},"enabled":True,"failures":0,"recoveryCount":0,"lastError":None,"lease":None,"checkpoint":None});d['version']=11
...
echo "PASS: worker v5 heartbeat, claim/execute/commit, checkpoints and lease release verified."
