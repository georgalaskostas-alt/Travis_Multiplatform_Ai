...
final class PersistenceService {
    ...
    private init() {
        do {
            ...
            // Production policy: never destroy a user's SwiftData store as an
            // automatic recovery action. If the schema/store cannot be opened,
            // fail closed and preserve the on-disk data for migration/support.
            // A versioned SchemaMigrationPlan must handle future schema changes.
            preconditionFailure("Unable to initialize the SwiftData ModelContainer without destructive recovery: \(error)")
        }
