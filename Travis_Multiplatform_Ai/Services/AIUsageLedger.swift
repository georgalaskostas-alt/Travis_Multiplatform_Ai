...
final class AIUsageLedger {
    ...
    func estimatedSpendUSD(since date: Date? = nil, taskId: UUID? = nil) -> Double {
        records.lazy
            .filter { record in
                (date == nil || record.timestamp >= date!) && (taskId == nil || record.taskId == taskId)
            }
    ...
    func usageSummary(since date: Date? = nil) -> AITokenUsage {
        records.lazy
            ...
            .reduce(into: AITokenUsage()) { result, record in
    ...
    private func pricingKey(provider: AIProvider, model: String) -> String {
        "\(provider.rawValue.lowercased())::\(model.lowercased())"
    }
}
