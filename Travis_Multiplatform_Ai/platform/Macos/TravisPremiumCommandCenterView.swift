        ...
        ZStack {
            ...
            ForEach(0..<11, id: \.self) { i in
                Circle().trim(from: i % 2 == 0 ? 0.02 : 0.16, to: i % 2 == 0 ? 0.84 : 0.97)
                    .stroke(
                        i < 5 ? cyan.opacity(0.98 - Double(i)*0.08) : blue.opacity(0.42),
                        style: StrokeStyle(lineWidth: i < 2 ? 3.6 : 1.6, lineCap: .round, dash: i % 3 == 0 ? [3,6] : [])
                    )
    ...
    private var coreGrid: some View {
        Canvas { context,size in
            ...
            for r in stride(from:CGFloat(60),through:255,by:26) {
                context.stroke(Path(ellipseIn:CGRect(x:c.x-r,y:c.y-r,width:r*2,height:r*2)), with: .color(cyan.opacity(0.085)), lineWidth: 0.8)
            }
            for a in stride(from:0.0,to:360.0,by:15.0) {
                ...
                context.stroke(p, with: .color(cyan.opacity(0.045)), lineWidth: 0.75)
            }
        }
    }
    ...
    private var premiumBackground: some View {
        ZStack {
            ...
            Canvas { context,size in
                ...
                context.stroke(p, with: .color(cyan.opacity(0.05)), lineWidth: 0.6)
            }
            RadialGradient(colors:[cyan.opacity(0.10),.clear],center:.center,startRadius:20,endRadius:680)
        }.ignoresSafeArea()
    }
