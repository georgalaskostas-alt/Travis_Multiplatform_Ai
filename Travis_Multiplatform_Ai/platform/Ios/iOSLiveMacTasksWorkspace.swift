...
struct iOSLiveMacTasksWorkspace: View {
    ...
    private var terminalMacTasks: [TravisBridgeTaskSnapshot] { macTasks.filter { ["completed", "failed", "cancelled"].contains(normalize($0.status)) } }
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            ScrollView {
                VStack(spacing: 13) {
                    ...
                    if usingMac {
                        if macTasks.isEmpty { emptyState(title: "NO MAC RUNTIME TASKS", detail: "Mac TRAVIS is connected. New missions will appear here live.") }
                        else { ForEach(macTasks) { macTaskCard($0) } }
                    } else {
                        if localTasks.isEmpty { emptyState(title: "NO LOCAL TASKS", detail: "Mac TRAVIS is offline. Showing the iPhone local runtime fallback.") }
                        else { ForEach(localTasks) { localTaskCard($0) } }
                    }
                    runtimeCard
                }.padding(14).padding(.bottom, 28)
            }
            if let completionToast { completionBanner(completionToast).padding(.horizontal, 14).padding(.top, 8).transition(.move(edge: .top).combined(with: .opacity)).zIndex(10) }
        }
        .navigationBarTitleDisplayMode(.inline)
        .task {
            requestNotificationAuthorization()
            while !Task.isCancelled {
                if bridge.isConnected { bridge.requestStatus(); try? await Task.sleep(for: .milliseconds(350)); processTerminalTaskChanges() }
        ...
        } message: { task in Text("Delete \(task.title) from the Mac TRAVIS runtime history?") }
        .alert("Delete finished task history?", isPresented: $showDeleteAllAlert) {
            Button("Delete Finished", role: .destructive) { deleteAllTasks() }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("This removes completed, failed and cancelled tasks only. Any active mission remains protected and keeps running.")
        }
    }

    private var header: some View {
        HStack(spacing: 12) {
            ZStack { RoundedRectangle(cornerRadius: 12).fill(cyan.opacity(0.09)).frame(width: 48, height: 48); RoundedRectangle(cornerRadius: 12).stroke(cyan.opacity(0.48), lineWidth: 1).frame(width: 48, height: 48); Image(systemName: "checklist").font(.system(size: 20, weight: .bold)).foregroundStyle(cyan) }
            VStack(alignment: .leading, spacing: 3) { Text("TASK CONTROL").font(.system(size: 19, weight: .heavy, design: .rounded)).tracking(1); Text(usingMac ? "CONNECTED MAC RUNTIME" : "LOCAL FALLBACK").font(.system(size: 8, weight: .bold, design: .rounded)).foregroundStyle(usingMac ? Color.green : Color.orange) }
            Spacer(); Circle().fill(usingMac ? Color.green : Color.orange).frame(width: 7, height: 7).shadow(color: usingMac ? .green : .orange, radius: 5)
        }.padding(14).liveTaskHUD(cyan: cyan, panel: panel)
    }

    private var summaryGrid: some View {
        let statuses = usingMac ? macTasks.map { normalize($0.status) } : localTasks.map { normalize($0.status.rawValue) }
        return LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
            summaryTile("RUNNING", statuses.filter { $0 == "running" || $0 == "planning" }.count, .green)
            summaryTile("WAITING", statuses.filter { ["waitingforapproval", "waitingfordependency", "paused", "pending"].contains($0) }.count, .orange)
            summaryTile("COMPLETED", statuses.filter { $0 == "completed" }.count, cyan)
            summaryTile("FAILED", statuses.filter { $0 == "failed" || $0 == "cancelled" }.count, .red)
        }
    }

    private var historyActions: some View {
        HStack(spacing: 10) {
            Label("ACTIVE MISSIONS PROTECTED", systemImage: "checkmark.shield.fill").font(.system(size: 8, weight: .bold, design: .rounded)).foregroundStyle(.green)
            Spacer()
            Button { showDeleteAllAlert = true } label: {
                Label("DELETE FINISHED (\(terminalMacTasks.count))", systemImage: "trash.fill").font(.system(size: 8, weight: .heavy, design: .rounded)).foregroundStyle(terminalMacTasks.isEmpty ? Color.secondary : Color.red).padding(.horizontal, 9).padding(.vertical, 6).background(Capsule().fill(Color.red.opacity(terminalMacTasks.isEmpty ? 0.03 : 0.09))).overlay(Capsule().stroke(Color.red.opacity(terminalMacTasks.isEmpty ? 0.10 : 0.35), lineWidth: 0.8))
            }.buttonStyle(.plain).disabled(terminalMacTasks.isEmpty)
        }.padding(11).liveTaskHUD(cyan: cyan, panel: panel)
    }

    private func emptyState(title: String, detail: String) -> some View {
        VStack(spacing: 9) { Image(systemName: "checkmark.circle").font(.system(size: 34, weight: .light)).foregroundStyle(cyan); Text(title).font(.system(size: 13, weight: .heavy, design: .rounded)); Text(detail).font(.system(size: 10, weight: .medium, design: .rounded)).foregroundStyle(.secondary).multilineTextAlignment(.center) }.frame(maxWidth: .infinity).padding(24).liveTaskHUD(cyan: cyan, panel: panel)
    }
    ...
    private func macTaskCard(_ task: TravisBridgeTaskSnapshot) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top) {
                ...
                Spacer(minLength: 8); statusBadge(task.status)
            }
            ...
            if ["completed", "failed", "cancelled"].contains(statusKey) {
                HStack { Spacer(); Button { pendingDeleteTask = task; showDeleteTaskAlert = true } label: { Label("DELETE TASK", systemImage: "trash").font(.system(size: 8, weight: .heavy, design: .rounded)).foregroundStyle(.red) }.buttonStyle(.plain) }
            }
        }.padding(13).liveTaskHUD(cyan: cyan, panel: panel)
    }

    @ViewBuilder private func remoteControls(_ task: TravisBridgeTaskSnapshot) -> some View {
        let key = normalize(task.status)
        HStack(spacing: 7) {
            switch key {
            case "running": controlButton("PAUSE", "pause.fill", .orange) { sendRemote("/remote-pause-task \(task.id.uuidString)") }; controlButton("CANCEL", "xmark", .red) { sendRemote("/remote-cancel-task \(task.id.uuidString)") }
            case "paused": controlButton("RESUME", "play.fill", .green) { sendRemote("/remote-resume-task \(task.id.uuidString)") }; controlButton("CANCEL", "xmark", .red) { sendRemote("/remote-cancel-task \(task.id.uuidString)") }
            case "waitingforapproval", "waitingfordependency": controlButton("PAUSE", "pause.fill", .orange) { sendRemote("/remote-pause-task \(task.id.uuidString)") }; controlButton("CANCEL", "xmark", .red) { sendRemote("/remote-cancel-task \(task.id.uuidString)") }
            case "failed": controlButton("RETRY", "arrow.clockwise", .orange) { sendRemote("/remote-retry-task \(task.id.uuidString)") }
            default: EmptyView()
            }
            Spacer()
        }
    ...
    private func localTaskCard(_ task: AgentTask) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 9) { HStack(alignment: .top) { VStack(alignment: .leading, spacing: 3) { Text(task.title).font(.system(size: 14, weight: .bold, design: .rounded)).lineLimit(2); Text(task.goal).font(.system(size: 10, weight: .medium, design: .rounded)).foregroundStyle(.secondary).lineLimit(3) }; Spacer(minLength: 8); statusBadge(task.status.rawValue) }; ProgressView(value: progress).tint(cyan); HStack { Text("\(completed)/\(total) STEPS • \(Int(progress * 100))%"); Spacer(); Text(task.priority.rawValue.uppercased()) }.font(.system(size: 8, weight: .bold, design: .rounded)).foregroundStyle(.secondary) }.padding(13).liveTaskHUD(cyan: cyan, panel: panel)
    }
    ...
    private func deleteTask(_ task: TravisBridgeTaskSnapshot) { pendingDeleteTask = nil; sendRemote("/remote-delete-task \(task.id.uuidString)") }
    private func deleteAllTasks() { sendRemote("/remote-delete-all-tasks") }
    ...
    private func processTerminalTaskChanges() {
        ...
        if !didSeedNotificationState { terminalTaskIDs = Set(terminal.map(\.id)); didSeedNotificationState = true; return }
        for task in terminal where !terminalTaskIDs.contains(task.id) { terminalTaskIDs.insert(task.id); let completed = normalize(task.status) == "completed"; let detail = completed ? (task.finalReport ?? task.checkpoint ?? "Mission completed successfully.") : (task.failureReason ?? task.checkpoint ?? "Mission stopped with an error."); postMissionNotification(task: task, completed: completed, detail: detail) }
        let currentIDs = Set(tasks.map(\.id)); terminalTaskIDs = terminalTaskIDs.filter { currentIDs.contains($0) }
    }
    private func postMissionNotification(task: TravisBridgeTaskSnapshot, completed: Bool, detail: String) {
        let content = UNMutableNotificationContent(); content.title = completed ? "TRAVIS Mission Ready" : "TRAVIS Mission Needs Attention"; content.subtitle = task.title; content.body = String(detail.replacingOccurrences(of: "\n", with: " ").prefix(180)); content.sound = .default
        UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "travis-mission-\(task.id.uuidString)-\(task.status)", content: content, trigger: nil))
        UINotificationFeedbackGenerator().notificationOccurred(completed ? .success : .error)
        withAnimation(.spring(response: 0.35, dampingFraction: 0.86)) { completionToast = completed ? "\(task.title) — completed" : "\(task.title) — needs attention" }
        Task { @MainActor in try? await Task.sleep(for: .seconds(4)); withAnimation(.easeOut(duration: 0.2)) { completionToast = nil } }
    }
    private func normalize(_ status: String) -> String { status.lowercased().filter { $0.isLetter } }
}

private extension View {
    func liveTaskHUD(cyan: Color, panel: Color) -> some View { background(RoundedRectangle(cornerRadius: 16).fill(LinearGradient(colors: [.white.opacity(0.05), panel.opacity(0.96), Color.black.opacity(0.78)], startPoint: .topLeading, endPoint: .bottomTrailing))).overlay(RoundedRectangle(cornerRadius: 16).stroke(LinearGradient(colors: [.white.opacity(0.16), cyan.opacity(0.48), .clear], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 1)).shadow(color: .black.opacity(0.65), radius: 12, y: 7) }
}
