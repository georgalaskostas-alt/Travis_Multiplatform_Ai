...
struct iOSLiveMacTasksWorkspace: View {
    ...
    private var summaryGrid: some View { let statuses = usingMac ? macTasks.map { normalize($0.status) } : localTasks.map { normalize($0.status.rawValue) }; return LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) { summaryTile("RUNNING", statuses.filter { $0 == "running" || $0 == "planning" }.count, .green); summaryTile("WAITING", statuses.filter { ["waitingforapproval", "waitingfordependency", "paused", "pending"].contains($0) }.count, .orange); summaryTile("COMPLETED", statuses.filter { $0 == "completed" }.count, cyan); summaryTile("FAILED", statuses.filter { $0 == "failed" }.count, .red) } }
