...
final class TRAVISAppState {
    ...
    init() {
        ...
        localAutomationCapability.onExecutionUpdate = { [weak self] text in
            self?.addAssistantMessage(text)
        }
    ...
    func viewSession(_ sessionId: UUID) {
        ...
        chatMessages = PersistenceService.shared
            .loadChatMessages()
    ...
    func refreshTradingMandates() {
        tradingMandates = PersistenceService.shared
            .standingPermissions(withKeyPrefix: "trading_")
            .filter { $0.granted }
    }
