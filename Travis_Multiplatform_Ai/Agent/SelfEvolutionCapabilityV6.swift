...
final class SelfEvolutionCapabilityV6: AgentCapability {
    ...
    var descriptor: CapabilityDescriptor {
        CapabilityDescriptor(
            id:id, displayName:name, summary:capabilityDescription, domain:.coding, keywords:keywords,
            policy:CapabilityExecutionPolicy(
                declaredEffects:[.readOnly,.codeMutation,.externalMutation], permissionKeys:["github_write"],
                requiresExplicitApproval:true, supportsBackgroundExecution:false, supportsProjectContext:true,
                timeoutSeconds:240, maxAttempts:2
            ),
            version:1,
            inputSchema:["path":"exact repository path using path=...","request":"desired self/GUI improvement"],
            outputKinds:[.code,.file], costClass:.cloudReasoning, deterministicWhenStructured:false, freshnessSensitive:true
        )
    }
    ...
    func handle(command:String,recentHistory:[ChatMessage]) async throws -> CapabilityOutcome {
        ...
        - Do not edit project.pbxproj from this capability.
        ...
        """
        let context=AIInvocationContext(workload:.frontier,capabilityId:id,taskId:AIExecutionScope.context.taskId,stepId:AIExecutionScope.context.stepId,projectId:AIExecutionScope.context.projectId,operation:"self_evolution.propose")
        let raw=try await aiService.generateText(prompt:prompt,maxTokens:16000,context:context)
        guard let fileOpen=raw.range(of:"<TRAVIS_FILE>"),let fileClose=raw.range(of:"</TRAVIS_FILE>",range:fileOpen.upperBound..<raw.endIndex),let metaOpen=raw.range(of:"<META>",range:fileClose.upperBound..<raw.endIndex),let metaClose=raw.range(of:"</META>",range:metaOpen.upperBound..<raw.endIndex) else {
        ...
        guard replacement.count > max(80,snapshot.content.count/5),!replacement.contains("<CURRENT_FILE>"),!replacement.contains("... unchanged"),!replacement.contains("TODO: existing code") else {
            return .reply("SELF-EVOLUTION V6 deterministic completeness guard rejected the proposed replacement.")
        }
        let metaText=String(raw[metaOpen.upperBound..<metaClose.lowerBound])
        guard let metaData=metaText.data(using:.utf8),let meta=try? JSONSerialization.jsonObject(with:metaData) as? [String:Any] else{return .reply("SELF-EVOLUTION V6 could not validate proposal metadata.")}
        ...
        let payload=MutationPayload(baseBranch:baseBranch,targetBranch:target,path:path,expectedBaseFileSHA:snapshot.sha,newContent:replacement,commitMessage:String(commit.prefix(180)),rationale:String(rationale.prefix(4000)),validationPlan:Array(validation.prefix(12)))
        let encoded=try JSONEncoder().encode(payload);guard let payloadText=String(data:encoded,encoding:.utf8)else{return .none}
    ...
    func resolve(_ action:ProposedAction) {
        guard action.status == .approved,let raw=action.payload,let data=raw.data(using:.utf8),let mutation=try? JSONDecoder().decode(MutationPayload.self,from:data)else{return}
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                let commitSHA=try await self.github.replaceFile(path:mutation.path,expectedSHA:current.sha,newContent:mutation.newContent,commitMessage:mutation.commitMessage,branchOverride:mutation.targetBranch)
                self.onExecutionUpdate?("✅ SELF-EVOLUTION candidate committed to isolated branch only: \(mutation.targetBranch) · \(String(commitSHA.prefix(10)))\nValidation still required before any merge: \(mutation.validationPlan.joined(separator:" · "))")
            } catch {
    ...
    private static func explicitValue(prefix:String,in text:String)->String? {
        for token in text.split(whereSeparator:{$0.isWhitespace || $0=="," || $0==";"}) {
            let s=String(token);guard s.hasPrefix(prefix)else{continue};let v=String(s.dropFirst(prefix.count)).trimmingCharacters(in:CharacterSet(charactersIn:"\"'"));if !v.isEmpty{return v}
        }
        return nil
    }
}
