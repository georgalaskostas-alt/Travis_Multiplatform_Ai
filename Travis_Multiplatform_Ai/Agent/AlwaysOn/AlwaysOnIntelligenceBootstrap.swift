...
enum AlwaysOnIntelligenceBootstrap {
    ...
    static func provision(monitor:AlwaysOnWorkerMonitor = .shared,persistence:PersistenceService = .shared){
        ...
        do{
            ...
            if !existing.contains(paperJobID){try monitor.enqueueCreateJob(id:paperJobID,title:"TRAVIS Autonomous PAPER Strategy",kind:"tradingPaper",payload:["assets":["BTC","ETH","SOL","XRP","BNB","ADA","DOGE","LINK"],"interval":"1h","riskPercent":0.005,"maxOpenPositions":3,"maxDailyLoss":500.0,"maxDrawdownPercent":15.0,"maxPortfolioExposurePercent":60.0,"maxPositionNotional":2000.0,"stopATRMultiple":1.8,"takeProfitATRMultiple":2.7,"minTrendScore":2.7,"minConfidence":0.68,"feeRate":0.001,"slippageBps":3.0,"cooldownSeconds":1800,"startingBalance":10000.0],cadenceSeconds:300)}
            // Continuous self-review is zero-token by default. AI escalation happens only when explicitly justified.
