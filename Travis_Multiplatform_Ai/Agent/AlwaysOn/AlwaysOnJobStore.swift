...
actor AlwaysOnJobStore {
    ...
    func upsert(_ job:AlwaysOnJob) throws { var jobs=try load(); if let i=jobs.firstIndex(where:{$0.id==job.id}){jobs[i]=job}else{jobs.append(job)}; try save(jobs) }
    func remove(_ id:UUID) throws { var jobs=try load(); jobs.removeAll{$0.id==id}; try save(jobs) }
}
