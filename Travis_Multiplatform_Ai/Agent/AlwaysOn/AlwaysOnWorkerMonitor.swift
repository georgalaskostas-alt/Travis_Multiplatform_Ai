...
final class AlwaysOnWorkerMonitor {
    ...
    func sendServiceJobCommand(action:String,jobID:UUID)throws{let allowed:Set<String>=["pause","resume","retry","delete"];guard allowed.contains(action.lowercased()) else{throw WorkerCommandError.unsupported};let payload:[String:Any]=["action":action.lowercased(),"jobID":jobID.uuidString,"nonce":UUID().uuidString,"createdAt":Date().timeIntervalSince1970];let data=try JSONSerialization.data(withJSONObject:payload,options:[.sortedKeys]);try data.write(to:commandURL,options:.atomic)}
