...
final class AlwaysOnWorkerMonitor {
    ...
    private let heartbeatURL:URL,controlURL:URL,commandQueueURL:URL
    ...
    func sendServiceJobCommand(action:String,jobID:UUID)throws{
        let allowed:Set<String>=["pause","resume","retry","delete"];guard allowed.contains(action.lowercased()) else{throw WorkerCommandError.unsupported}
        ...
        queue.append(["action":action.lowercased(),"jobID":jobID.uuidString,"nonce":UUID().uuidString,"createdAt":Date().timeIntervalSince1970])
        if queue.count>200{queue=Array(queue.suffix(200))}
        let payload:[String:Any]=["version":1,"commands":queue];let data=try JSONSerialization.data(withJSONObject:payload,options:[.sortedKeys]);try data.write(to:commandQueueURL,options:.atomic)
    }
