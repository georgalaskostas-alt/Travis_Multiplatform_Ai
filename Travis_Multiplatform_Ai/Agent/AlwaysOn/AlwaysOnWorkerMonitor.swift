...
final class AlwaysOnWorkerMonitor {
    ...
    private let heartbeatURL:URL,controlURL:URL,legacyCommandURL:URL,commandQueueURL:URL,serviceJobsURL:URL
    ...
    func serviceJobID(forSourceTaskID taskID:UUID)->UUID?{
        guard let data=try? Data(contentsOf:serviceJobsURL),let root=try? JSONSerialization.jsonObject(with:data) as? [String:Any],let jobs=root["jobs"] as? [[String:Any]] else{return nil}
        let key=taskID.uuidString.lowercased();let matches=jobs.compactMap{job->UUID? in guard (job["kind"] as? String)=="headlessMission",let payload=job["payload"] as? [String:Any],let source=(payload["sourceTaskID"] as? String)?.lowercased(),source==key,let raw=job["id"] as? String else{return nil};return UUID(uuidString:raw)}
        return matches.last
    }
