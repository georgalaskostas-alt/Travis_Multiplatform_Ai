...
final class VerifiedLearningStore {
    ...
    func ingestCompletedTask(_ task: AgentTask, projectId: UUID?) {
        ...
        for step in task.plan.steps where step.status == .completed {
            ...
            examples.append(Example(
                ...
                title: step.title,
                instruction: String(step.instructions.prefix(12_000)),
                successCriteria: step.successCriteria,
                verifiedResult: String(result.prefix(16_000)),
    ...
    func recentExamples(
        ...
        return examples
            .reversed()
            .filter { example in
                (capabilityId == nil || example.capabilityId == capabilityId) &&
                (projectId == nil || example.projectId == projectId)
            }
            .prefix(bounded)
            .map { $0 }
    }
    ...
    func diagnosticReport() -> String {
        ...
        let rows = grouped
            .map { key, values in "\(key): \(values.count) verified examples" }
            .sorted()
            .joined(separator: "\n")
