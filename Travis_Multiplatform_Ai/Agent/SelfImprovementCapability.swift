...
final class SelfImprovementCapability: AgentCapability {
    ...
    func handle(command: String, recentHistory: [ChatMessage]) async throws -> CapabilityOutcome {
        ...
        let prompt = """
        Είσαι ο TRAVIS, προσωπικός AI βοηθός. Ο ίδιος ο TRAVIS είναι μια εφαρμογή SwiftUI (iOS/macOS/visionOS), ενιαίο Xcode target, της οποίας ο πηγαίος κώδικας αναπτύσσεται από τον χρήστη σε συνεργασία με το Claude Code.

        Τεχνικό context του codebase — ΧΡΗΣΙΜΟΠΟΙΗΣΕ ΑΥΤΟ ρητά σε ό,τι προτείνεις, ΟΧΙ γενικές best practices από άλλες γλώσσες/frameworks (ΠΟΤΕ React, JavaScript/TypeScript, useState, signals, hooks, ή οτιδήποτε εκτός Swift/SwiftUI/SwiftData — αυτό είναι αμιγώς Swift codebase):
        - Γλώσσα/UI: Swift + SwiftUI· κάποια λίγα platform-specific σημεία σε platform/Ios/ και platform/Macos/, τίποτα άλλο σε UIKit/AppKit απευθείας.
        - State management: @Observable + @MainActor classes (π.χ. TRAVISAppState, AgentOrchestrator, ApprovalGateService, SpeechService) — όχι Combine, όχι ObservableObject/@Published.
        - Persistence: SwiftData, @Model classes στο Models/ (π.χ. PersistedChatMessage, StandingPermission, PersistedPaperAccount), προσβάσιμα μόνο μέσω του PersistenceService singleton (Services/PersistenceService.swift). Synced με CloudKit (ModelConfiguration cloudKitDatabase: .automatic) — νέα/τροποποιημένα @Model properties χρειάζονται πάντα inline default value, ΠΟΤΕ @Attribute(.unique).
        - Agent αρχιτεκτονική: κάθε δυνατότητα είναι class που υλοποιεί το AgentCapability protocol (handle(command:recentHistory:) / resolve(_:)) μέσα στο Agent/, καταχωρημένη στο AgentOrchestrator μέσω keyword-based routing στο TRAVISAppState.init(). Οτιδήποτε έχει side-effect γίνεται πάντα ProposedAction μέσω του ApprovalGateService — ποτέ απευθείας εκτέλεση χωρίς έγκριση.
        - Standing permissions: γενικό key-based μοντέλο (StandingPermission + PersistenceService.isPermissionGranted/setPermission) για οτιδήποτε χρειάζεται εφάπαξ συναίνεση του χρήστη (π.χ. "file_save", "trading_XRP").
        - UI: κύριο chat view στο Features/Chat/ChatView.swift, ρυθμίσεις στο Features/Settings/SettingsView.swift.
        - Δομή project: Agent/, Models/, Services/, Features/, App/, platform/. Τα Agent/, Features/, App/, platform/ είναι Xcode 16 file-system-synchronized groups (νέα αρχεία εκεί ΔΕΝ χρειάζονται pbxproj wiring)· τα Models/ και Services/ είναι classic PBXGroup (νέα αρχεία ΧΡΕΙΑΖΟΝΤΑΙ manual wiring στο project.pbxproj — PBXBuildFile, PBXFileReference, group children, Sources build phase).
        \(historyBlock)Ο χρήστης έγραψε τώρα: "\(command)"
        ...
        Αν ΕΙΝΑΙ γνήσιο αίτημα, απάντησε ΑΠΟΚΛΕΙΣΤΙΚΑ με JSON σε αυτή τη μορφή, χωρίς κανένα άλλο κείμενο πριν ή μετά:
        {
          ...
          "claudeCodePrompt": "ΠΛΗΡΕΣ, αναλυτικό prompt στα ελληνικά, έτοιμο να δοθεί απευθείας στο Claude Code — ΟΧΙ γενικόλογο, ΠΑΝΤΑ grounded στο πραγματικό codebase (πραγματικά ονόματα αρχείων/types/patterns από το τεχνικό context παραπάνω· ΠΟΤΕ pseudocode ή concepts από άλλη γλώσσα/framework). Αν δεν είσαι σίγουρος για το ακριβές σημείο ή την ακριβή αιτία του προβλήματος, το prompt ΠΡΕΠΕΙ να ζητάει ρητά από το Claude Code να διερευνήσει πρώτα το πραγματικό codebase πριν προτείνει διόρθωση — συγκεκριμένες εντολές (π.χ. 'κάνε grep για Χ', 'διάβασε το αρχείο Υ') αντί να υποθέσει λύση χωρίς επιβεβαίωση, ακριβώς όπως γίνεται σε πραγματικές συνομιλίες ανάμεσα στον χρήστη και το Claude Code όταν η αιτία δεν είναι γνωστή εκ των προτέρων. Πρέπει να περιλαμβάνει: (1) ακριβή περιγραφή του προβλήματος/της αλλαγής, με αριθμημένα βήματα όπου έχει νόημα (συμπεριλαμβανομένων βημάτων διερεύνησης αν χρειάζεται, όπως παραπάνω), (2) ρητή οδηγία να κάνει πρώτα git fetch και merge --ff-only με το origin branch πριν αγγίξει οτιδήποτε, (3) ρητή οδηγία να ακολουθήσει τα υπάρχοντα patterns/conventions του project (βλ. τεχνικό context παραπάνω) αντί να εισάγει νέα, (4) ρητή οδηγία να τρέξει τον καθιερωμένο target-membership έλεγχο (brace/paren balance σε project.pbxproj και σε όλα τα Swift αρχεία, occurrence-count έλεγχο για τυχόν νέα object IDs, single-native-target έλεγχο) πριν από οποιοδήποτε commit, (5) ρητή οδηγία για commit με περιγραφικό μήνυμα και push. Γράψε το με την ίδια λεπτομέρεια και ύφος που θα είχε μια πραγματική εντολή χρήστη προς το Claude Code."
        }
        """
