...
@MainActor final class AutonomousMissionPlannerV2{
 ...
 func makePlan(goal:String,capabilities:[AgentCapability],priorKnowledge:String?=nil)async throws->TaskPlan{
  ...
  if let localPlan=deterministicPlanIfAvailable(goal:g,capabilities:capabilities){return localPlan}
 ...
 private func deterministicRuntimeDiagnosticsPlan(goal:String,available:Set<String>)->TaskPlan?{
  let normalized=goal.folding(options:[.diacriticInsensitive,.caseInsensitive],locale:Locale(identifier:"el_GR")).lowercased();let runtimeIntent=normalized.contains("runtime") || normalized.contains("travis runtime");let diagnosticIntent=["identity","ταυτοτητα","health","υγεια","safety","ασφαλ","report","αναφορα","status","κατασταση"].contains(where:normalized.contains);let monitoringIntent=["monitor","monitoring","continuously","periodically","for 10 minutes","for 5 minutes","παρακολουθ","συνεχ","περιοδικ"].contains(where:normalized.contains);if monitoringIntent{return nil};let required:Set<String>=["runtime_identity","runtime_health","runtime_safety","report_synthesis"];guard runtimeIntent && diagnosticIntent && required.isSubset(of:available) else{return nil}
