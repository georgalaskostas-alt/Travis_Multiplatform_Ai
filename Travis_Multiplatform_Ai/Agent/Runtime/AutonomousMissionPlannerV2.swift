...
@MainActor final class AutonomousMissionPlannerV2{
 ...
 func makePlan(goal:String,capabilities:[AgentCapability],priorKnowledge:String?=nil)async throws->TaskPlan{
  ...
  let prompt="""
  You are the TRAVIS Cognitive Mission Planner V2. Build the smallest reliable executable end-to-end plan.
  USER GOAL:\n\(g)
  ...
  - Code/GUI mutation must respect approval-gated coding/self-improvement policy. Inspect before mutation, test after mutation.
 ...
 func makeRecoveryPlan(task:AgentTask,capabilities:[AgentCapability])async throws->TaskPlan{
  let completed=task.plan.steps.filter{$0.status == .completed}.sorted{$0.order<$1.order}.map{"STEP #\($0.order) \($0.title): \(String(($0.resultSummary ?? "No result").prefix(4000)))"}.joined(separator:"\n"),failed=task.plan.steps.first{$0.status == .failed},failure=failed.map{"STEP #\($0.order) \($0.title): \($0.lastError ?? task.failureReason ?? "Unknown")"} ?? (task.failureReason ?? "Unknown"),registry=CapabilityRegistry(capabilities:capabilities),headless=Self.headlessCatalog(capabilities)
  let prompt="""
  You are TRAVIS Self-Correction Planner V2. Diagnose the failure and produce only the remaining work for ORIGINAL GOAL: \(task.goal)
