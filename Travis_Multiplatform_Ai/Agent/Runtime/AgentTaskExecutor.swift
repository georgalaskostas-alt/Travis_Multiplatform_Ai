...
@MainActor @Observable final class AgentTaskExecutor{
    ...
    func isTaskExecuting(_ id:UUID)->Bool{leasedTaskIds.contains(id)}
    ...
    @discardableResult private func executeNextStepWithLease(taskId:UUID,recentHistory:[ChatMessage])async throws->PlanStep?{
        ...
        do{
            ...
            let skillMatch=LocalSkillMemory.shared.bestMatch(instruction:instruction,capabilityId:capabilityId,projectId:projectId)
            if let skillMatch{LocalIntelligenceMetrics.shared.record(.learnedExecutionGuidance);onProgress?("⚡ Χρησιμοποιώ τοπική επιβεβαιωμένη δεξιότητα (\(Int(skillMatch.confidence*100))%).")}
            ...
            case .reply(let text):
                ...
                case .insufficientEvidence:if capabilityId=="repository_context"{let limited="\(text)\n\nVERIFICATION LIMITATION\n\(verification.reason)\nUnmet scope: \(verification.unmetCriteria.joined(separator:" | "))";runtime.markStepCompleted(taskId:taskId,stepId:step.id,resultSummary:limited);return runtime.nextRunnableStep(taskId:taskId)};let r=verification.reason;runtime.markStepFailed(taskId:taskId,stepId:step.id,error:r);FailureLearningStore.shared.record(capabilityId:capabilityId,instruction:instruction,error:r,projectId:projectId);throw AgentTaskExecutorError.verificationFailed(r)
                case .retry:let r=verification.reason;runtime.markStepFailed(taskId:taskId,stepId:step.id,error:r);FailureLearningStore.shared.record(capabilityId:capabilityId,instruction:instruction,error:r,projectId:projectId);throw AgentTaskExecutorError.verificationFailed(r)}
            case .proposal(let action):try throwIfCancellationRequested(taskId);approvalGate.submit(action);runtime.markStepWaitingForApproval(taskId:taskId,stepId:step.id);return step
            case .none:throw AgentTaskExecutorError.emptyCapabilityResult}
        }catch is CancellationError{runtime.pause(taskId:taskId,reason:"Execution cancelled before verified completion");throw CancellationError()}
        catch let error as UniversalCapabilityRunner.RunnerError{if case .timedOut(let seconds)=error{let mapped=AgentTaskExecutorError.capabilityTimedOut(seconds:seconds);FailureLearningStore.shared.record(capabilityId:capabilityId,instruction:step.title+" "+step.instructions,error:mapped.localizedDescription,projectId:projectId);if let t=runtime.task(id:taskId),let s=t.plan.steps.first(where:{$0.id==step.id}),s.status == .running{runtime.markStepFailed(taskId:taskId,stepId:step.id,error:mapped.localizedDescription)};throw mapped};FailureLearningStore.shared.record(capabilityId:capabilityId,instruction:step.title+" "+step.instructions,error:error.localizedDescription,projectId:projectId);throw error}
        catch{FailureLearningStore.shared.record(capabilityId:capabilityId,instruction:step.title+" "+step.instructions,error:error.localizedDescription,projectId:projectId);if let t=runtime.task(id:taskId),let s=t.plan.steps.first(where:{$0.id==step.id}),s.status == .running{runtime.markStepFailed(taskId:taskId,stepId:step.id,error:error.localizedDescription)};lastExecutionSummary=error.localizedDescription;onProgress?("\(trace)\n❌ Step #\(step.order) απέτυχε: \(error.localizedDescription)");throw error}
    }

    func executeUntilBlocked(taskId:UUID,recentHistory:[ChatMessage]=[],maxStepsPerCycle:Int=8)async throws->AutonomousRunReport{try acquireExecutionLease(taskId:taskId);defer{releaseExecutionLease(taskId:taskId)};guard let initial=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};let retry=max(1,initial.budget.maxRetriesPerStep);let safeLimit=min(max(max(1,maxStepsPerCycle),max(1,initial.plan.steps.count*retry)),60);var attempted=0;while attempted<safeLimit{try throwIfCancellationRequested(taskId);guard let task=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};switch task.status{case .completed:return makeRunReport(task:task,reason:.completed,stepsAttempted:attempted);case .waitingForApproval:return makeRunReport(task:task,reason:.waitingForApproval,stepsAttempted:attempted);case .paused:return makeRunReport(task:task,reason:.paused,stepsAttempted:attempted);case .failed,.cancelled:return makeRunReport(task:task,reason:.failed,stepsAttempted:attempted);case .pending,.planning,.waitingForDependency:return makeRunReport(task:task,reason:.noRunnableStep,stepsAttempted:attempted);case .running:break};if let r=budgetViolationReason(task){runtime.pause(taskId:taskId,reason:"Execution budget exhausted: \(r)");guard let p=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};return makeRunReport(task:p,reason:.budgetExceeded,stepsAttempted:attempted)};guard runtime.nextRunnableStep(taskId:taskId) != nil else{guard let latest=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};return makeRunReport(task:latest,reason:.noRunnableStep,stepsAttempted:attempted)};do{_=try await executeNextStepWithLease(taskId:taskId,recentHistory:recentHistory);attempted+=1}catch is CancellationError{guard let l=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};return makeRunReport(task:l,reason:.paused,stepsAttempted:attempted)}catch{attempted+=1;guard let latest=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};if latest.status == .running,let retryStep=runtime.nextRunnableStep(taskId:taskId){let seconds=min(Int(pow(2.0,Double(max(0,retryStep.attemptCount-1)))),8);if seconds>0{try? await Task.sleep(for:.seconds(seconds))};continue};if latest.status == .failed || latest.status == .cancelled{return makeRunReport(task:latest,reason:.failed,stepsAttempted:attempted)};throw error};await Task.yield()};guard let latest=runtime.task(id:taskId)else{throw AgentTaskExecutorError.taskNotFound};return makeRunReport(task:latest,reason:.safetyStepLimitReached,stepsAttempted:attempted)}
    ...
    private func executionCommand(task:AgentTask,step:PlanStep,learnedGuidance:LearnedExecutionRegistry.Guidance?,learnedSkill:LocalSkillMemory.Match?,failureWarning:String?)->String{let criteria=step.successCriteria.enumerated().map{"\($0.offset+1). \($0.element)"}.joined(separator:"\n");let deps=dependencyEvidenceBlock(task:task,step:step);let learned:String;if let g=learnedGuidance{learned="VERIFIED PRIOR EXPERIENCE (guidance only):\nconfidence: \(Int(g.confidence*100))%\nprior instruction: \(g.priorInstruction)\nprior verified result: \(g.priorVerifiedResult)\nRe-run against current evidence; never copy blindly."}else{learned="No sufficiently similar verified prior experience."};let skill=learnedSkill.map{"LOCAL VERIFIED SKILL\nconfidence: \(Int($0.confidence*100))%\ncapability: \($0.example.capabilityId)\nprior instruction: \($0.example.instruction)\nverified result: \($0.example.verifiedResult)\nmatched terms: \($0.matchedTerms.joined(separator:", "))"} ?? "No sufficiently proven local skill.";let failures=failureWarning ?? "No strongly similar known failure.";return """
...
final class AgentStepVerifier{
    ...
    func verify(taskGoal:String,step:PlanStep,capabilityResult:String)async throws->StepVerificationResult{
        ...
        if let learned=LearnedVerificationRegistry.shared.verify(step:step,capabilityResult:capabilityResult){await MainActor.run{LocalIntelligenceMetrics.shared.record(.learnedVerification)};return learned}
        ...
        Verify ONLY the current step. Judge only evidence in the produced result.
        OVERALL GOAL: \(taskGoal)
        CURRENT STEP: \(step.title)
        INSTRUCTIONS: \(step.instructions)
        SUCCESS CRITERIA: \(criteria)
        PRODUCED RESULT: \(capabilityResult)
        Return ONLY JSON:
        {"verdict":"pass|retry|insufficient_evidence","confidence":0.0,"reason":"short reason","unmetCriteria":[]}
        """
        var lastRaw="";var diagnostic="unknown decode error";for attempt in 1...maxDecodeAttempts{try Task.checkCancellation();let prompt=attempt==1 ? base:"Repair into valid verifier JSON only: \(lastRaw)";let raw=try await aiService.generateText(prompt:prompt,maxTokens:attempt==1 ? 1200:500);lastRaw=raw;do{return try decodeVerifierResult(raw)}catch{diagnostic=error.localizedDescription}};throw AgentTaskExecutorError.verificationFailed("Verifier returned malformed JSON. \(diagnostic)")
    }
