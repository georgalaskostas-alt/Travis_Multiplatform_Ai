...
final class AgentTaskRuntime {
    ...
    func dispatchableTasks(backgroundOnly: Bool = false, limit: Int = 8, now: Date = Date()) -> [AgentTask] {
        ...
        return tasks.filter { task in
            ...
            if let nextEligible = task.executionState.nextEligibleRunAt, nextEligible > now { return false }
            guard let step = nextRunnableStep(taskId: task.id, now: now) else { return false }
            return !backgroundOnly || step.canRunInBackground
        }
    ...
    func staleRunningTasks(heartbeatOlderThan interval: TimeInterval, now: Date = Date()) -> [AgentTask] {
    ...
    func prepareRetry(taskId: UUID) -> Bool {
        ...
        mutate(taskId) { task in
            guard task.status == .failed,
                  let index = task.plan.steps.firstIndex(where: { $0.status == .failed }) else { return }
            task.plan.steps[index].status = .pending
            task.plan.steps[index].attemptCount = 0
            task.plan.steps[index].lastError = nil
            task.plan.steps[index].completedAt = nil
            task.failureReason = nil
            ...
            task.executionState.lastHeartbeatAt = Date()
            task.status = .running
            task.events.append(TaskEvent(type: .retry, message: "Explicit retry prepared for step \(task.plan.steps[index].order): \(task.plan.steps[index].title)"))
            prepared = true
        }
        return prepared
    }
    ...
    private func nextRunnableStep(taskId: UUID, now: Date) -> PlanStep? {
        ...
        return task.plan.steps.sorted { $0.order < $1.order }.first { step in
    ...
    func markStepRunning(taskId: UUID, stepId: UUID) {
        mutate(taskId) { task in
            guard task.status == .running, let index = task.plan.steps.firstIndex(where: { $0.id == stepId }) else { return }
    ...
    func markStepCompleted(taskId: UUID, stepId: UUID, resultSummary: String? = nil) {
        mutate(taskId) { task in
            ...
            if unfinished { task.status = .running }
            else {
    ...
    func markStepApprovalGranted(taskId: UUID, stepId: UUID) {
        mutate(taskId) { task in
            guard let index = task.plan.steps.firstIndex(where: { $0.id == stepId }), task.plan.steps[index].status == .waitingForApproval else { return }
    ...
    func checkpoint(taskId: UUID, summary: String, nextAction: String? = nil) {
        mutate(taskId) { task in
            let checkpoint = TaskCheckpoint(taskId: task.id, stepId: task.executionState.currentStepId, summary: summary, nextAction: nextAction)
    ...
    private func priorityRank(_ priority: AgentTaskPriority) -> Int {
        switch priority { case .low: return 0; case .medium: return 1; case .high: return 2; case .critical: return 3 }
    }
    ...
    private func persist() {
        ...
        catch { persistenceError = error.localizedDescription; print("TRAVIS runtime persistence failed: \(error.localizedDescription)") }
    }
    ...
    private func restorePersistedTasks() {
        do {
            ...
            for taskIndex in restored.indices {
                ...
                restored[taskIndex].events.append(TaskEvent(type: .paused, message: "Recovered from durable snapshot after process interruption"))
