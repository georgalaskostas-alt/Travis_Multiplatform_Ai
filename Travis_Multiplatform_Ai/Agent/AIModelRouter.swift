...
struct AIModelRouter {
    func selection(
        ...
        if hasOpenAI {
            switch workload {
            case .classification:
                return AIModelSelection(provider: .openAI, model: "gpt-5.6-luna", tier: .economy, reasoningEffort: "low", rationale: "Low-cost routing/classification workload")
            case .routine:
                return AIModelSelection(provider: .openAI, model: "gpt-5.6-luna", tier: .standard, reasoningEffort: "low", rationale: "Routine assistant workload")
            case .complex, .verification:
                return AIModelSelection(provider: .openAI, model: "gpt-5.6-terra", tier: .strong, reasoningEffort: "medium", rationale: "Complex planning/repository/verification workload")
            case .frontier, .webResearch:
                return AIModelSelection(provider: .openAI, model: "gpt-5.6-terra", tier: .frontier, reasoningEffort: "medium", rationale: "Frontier or web-grounded workload")
            case .deterministic:
                return nil
            }
        }
        ...
        if hasAnthropic {
            return AIModelSelection(provider: .anthropic, model: "claude-sonnet-4-6", tier: workload == .classification ? .standard : .strong, reasoningEffort: nil, rationale: "Anthropic fallback because OpenAI is unavailable")
        }
    ...
    func fallbackSelection(after primary: AIModelSelection, hasAnthropic: Bool) -> AIModelSelection? {
        ...
        return AIModelSelection(provider: .anthropic, model: "claude-sonnet-4-6", tier: .strong, reasoningEffort: nil, rationale: "Cross-provider fallback after primary failure")
    }
